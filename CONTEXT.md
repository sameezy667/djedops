...
- **Details**:
  ...
  - Retrieves connected account address: `2b32c1...db482a` (Account1)
  - **IMPORTANT**: WAuth is READ-ONLY in browser - cannot send transactions via `sendTransaction()`
  - All deployment must use CLI tool (widl)

#### 2. Visual Workflow Builder
- **Location**: `components/WorkflowBuilder.tsx`
- **Status**: Fully functional with CLI deployment
- **Features**:
  - Drag-and-drop node editor using React Flow
  - Node types: Trigger, Condition, Action, Teleport (cross-chain)
  - Real-time workflow validation
  - JSON export for CLI deployment
  - Shows CLI modal with deployment instructions when WAuth detected
- **Deployment Flow**:
  ...
  3. Modal shows with:
     ...
     - Setup instructions for widl tool

#### 3. CLI Deployment Helper
- **Location**: `lib/weil-cli-helper.ts`
- **Status**: Commands generated, CLI tool needs proper Windows binary
- **Functions**:
  - `generateCLIDeploymentCommand()` - Creates deployment commands
  - `copyToClipboard()` - Copy commands to clipboard
  - `downloadAsFile()` - Download workflow JSON
- **Current Commands** (need correction):
  ```bash
  # Configure RPC
  widl config set rpc-url https://sentinel.unweil.me
  
  # Import account
  widl account import <PRIVATE_KEY>
  
  # Deploy workflow
  widl contract call weil1coordinator00000000000000000000000 deploy_workflow --args workflow.json
  ```

#### 4. AI-Powered Semantic Command Bar
...
### Issue #1: WAuth Cannot Send Transactions from Browser
**Problem**: WAuth browser extension (`window.WeilWallet`) only provides account viewing. All transaction methods timeout:
- `wallet.request({ method: 'eth_sendTransaction' })` - Timeout
- `wallet.sendTransaction()` - Not available
- `wallet.WM.sendTransaction()` - Timeout

**Investigation Done**:
- Checked 10+ property paths on wallet object
- Tried Web3, EIP-1193, and custom request methods
- Confirmed wallet is connected: `isConnected: true`, `isUnlocked: true`
- Extension shows "1 accounts connected" but no transaction popup

**Solution Implemented**: CLI-based deployment workflow
- Generate deployment commands in browser
- User downloads workflow JSON
- User runs widl CLI tool to deploy
- See `lib/weil-cli-helper.ts` for implementation

**Status**: CLI workflow functional, but widl binary issue (see Issue #2)

### Issue #2: widl CLI Tool Binary Incompatibility
**Problem**: Downloaded `widl-x86_64-pc-windows-msvc.zip` contains Linux binary, not Windows .exe

**Downloaded File**: 
- Location: `C:\Users\samee\Downloads\widl.x86\widl`
- Issue: When run on Windows, triggers "Select an app to open" dialog
- Renamed to `widl.exe` but still fails: "The operation was canceled by the user"

**Evidence**: 
- `wsl --version` shows WSL not installed
- File doesn't have Windows PE header

**Required Actions**:
1. **Option A**: Install WSL (Windows Subsystem for Linux)
   ```powershell
   wsl --install
   # Restart computer
   # Then run: wsl widl --version
   ```

2. **Option B**: Find correct Windows binary
   - Check WeilChain docs for Windows release
   - Look for `widl-windows.zip` or `widl.exe` download
   - May need to compile from source for Windows

**Workaround**: Use Linux VM or WSL to run widl commands
...
2. Update `.env`:
   ...
   ```
3. Update `lib/weil-config.ts` with real address
